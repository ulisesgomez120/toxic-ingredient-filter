...
CREATE OR REPLACE FUNCTION "public"."update_normalized_columns"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
    NEW.normalized_brand := normalize_text(NEW.brand);
    NEW.normalized_base_name := normalize_text(NEW.base_name);
    RETURN NEW;
END;
$$;


ALTER FUNCTION "public"."update_normalized_columns"() OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."products"
    ADD CONSTRAINT "products_product_group_id_name_key" UNIQUE ("product_group_id", "name");
...
ALTER TABLE ONLY "public"."product_groups"
    ADD CONSTRAINT "unique_normalized_names" UNIQUE ("normalized_brand", "normalized_base_name");
...
CREATE INDEX "idx_product_groups_normalized" ON "public"."product_groups" USING "btree" ("normalized_brand", "normalized_base_name");



CREATE INDEX "idx_product_groups_normalized_base_name" ON "public"."product_groups" USING "btree" ("normalized_base_name");
...
CREATE OR REPLACE TRIGGER "product_groups_normalize_trigger" BEFORE INSERT OR UPDATE ON "public"."product_groups" FOR EACH ROW EXECUTE FUNCTION "public"."update_normalized_columns"();
...
ALTER TABLE ONLY "public"."products"
    ADD CONSTRAINT "products_product_group_id_fkey" FOREIGN KEY ("product_group_id") REFERENCES "public"."product_groups"("id");
...
GRANT ALL ON FUNCTION "public"."update_normalized_columns"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_normalized_columns"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_normalized_columns"() TO "service_role";
