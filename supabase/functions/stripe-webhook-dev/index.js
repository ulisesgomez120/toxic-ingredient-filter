...
async function handleSubscriptionCreated(subscription) {
  ...
  try {
    ...
    if (!customer || !customer.email) {
      throw new Error(`No customer email found for customer: ${subscription.customer}`);
    }

    // Create an invoice-like object with the customer email
    const invoiceLike = {
    ...
    // Use the existing function to create the subscription
    await createSubscriptionFromInvoice(invoiceLike, subscription);
    console.log("Successfully processed subscription creation");
  } catch (error) {
    console.error("Failed to handle subscription creation:", error);
    throw new Error(`Failed to handle subscription creation: ${error.message}`);
  }
}
...
async function processEvent(event) {
  ...
  switch (event.type) {
    case "customer.subscription.created":
      console.log("Detected subscription created event");
    ...
    case "invoice.created":
      // For invoice.created events with billing_reason "subscription_create"
      if (eventObject.billing_reason === "subscription_create" && eventObject.subscription) {
        console.log("Detected invoice created for subscription creation");
        // We'll handle this through the customer.subscription.created event instead
        console.log("This will be handled by the subscription.created event");
      } else {
        console.log(`Skipping invoice.created with billing_reason: ${eventObject.billing_reason}`);
      }
      break;
